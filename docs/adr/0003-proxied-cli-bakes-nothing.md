...
On `nixos-26.05-cli-determinate-proxy` the ISO is thin: no target
...
or injects anything into what it installs. The reason is the parent branch's
hardest-won lesson (ADR 0001 on `nixos-26.05-cli-determinate`, finding #6):
...
- This branch cannot install anything real until the appliance grows the
